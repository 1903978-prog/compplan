...
10. **Org chart layout is TOP-DOWN VERTICAL — each child must be below its parent.** Never use a left-to-right (right-stack) layout where children sit to the right of the parent. Children stack vertically below the parent in an indented-tree (file-explorer) pattern: every node on its own row, indented by depth, with a vertical bracket connector. Direct-reports groups are a vertical column, never a horizontal row. This is final — do not "fix" or "reorganize" it.
