...
// ── AIOS Daily Cycle ─────────────────────────────────────────────────────────
// status: not_started | running | paused | completed | failed | cowork_output_received
export const aiosCycles = pgTable("aios_cycles", {
  id:                    serial("id").primaryKey(),
  cycle_date:            text("cycle_date").notNull(),
  cycle_type:            text("cycle_type").notNull().default("daily"),
  status:                text("status").notNull().default("not_started"),
  started_at:            text("started_at"),
  completed_at:          text("completed_at"),
  started_by:            text("started_by").notNull().default("President"),
  summary:               text("summary"),
  cowork_prompt:         text("cowork_prompt"),
  cowork_output_raw:     text("cowork_output_raw"),
  agents_processed:      integer("agents_processed").notNull().default(0),
  sections_analyzed:     integer("sections_analyzed").notNull().default(0),
  insights_count:        integer("insights_count").notNull().default(0),
  ideas_count:           integer("ideas_count").notNull().default(0),
  actions_count:         integer("actions_count").notNull().default(0),
  cowork_requests_count: integer("cowork_requests_count").notNull().default(0),
  conflicts_count:       integer("conflicts_count").notNull().default(0),
  created_at:            text("created_at").notNull(),
  updated_at:            text("updated_at").notNull(),
});
export type AiosCycle = typeof aiosCycles.$inferSelect;

// actor_type: system | agent | boss | ceo | president
// status:     working | completed | warning | blocked | failed
// severity:   info | warning | critical
export const aiosExecLogs = pgTable("aios_exec_logs", {
  id:          serial("id").primaryKey(),
...
export type AiosExecLog = typeof aiosExecLogs.$inferSelect;

// deliverable_type: insight | idea | action | cowork_request
// decision_right_level: autonomous | boss_approval | ceo_approval | president_approval
export const aiosDeliverables = pgTable("aios_deliverables", {
  id:                   serial("id").primaryKey(),
  cycle_id:             integer("cycle_id").notNull(),
  agent_id:             integer("agent_id").notNull(),
  agent_name:           text("agent_name"),
  deliverable_type:     text("deliverable_type").notNull(),
  rank:                 integer("rank").notNull().default(1),
  title:                text("title").notNull(),
  description:          text("description"),
  source_app_section:   text("source_app_section"),
  okr_link:             text("okr_link"),
  okr_relevance_score:  integer("okr_relevance_score"),
  business_impact_score:integer("business_impact_score"),
  urgency_score:        integer("urgency_score"),
  confidence_score:     integer("confidence_score"),
  feasibility_score:    integer("feasibility_score"),
  total_score:          integer("total_score"),
  scoring_rationale:    text("scoring_rationale"),
  decision_right_level: text("decision_right_level").default("autonomous"),
  deadline:             text("deadline"),
...
export const bossConsolidations = pgTable("boss_consolidations", {
  id:                        serial("id").primaryKey(),
  cycle_id:                  integer("cycle_id").notNull(),
  boss_agent_id:             integer("boss_agent_id").notNull(),
  boss_agent_name:           text("boss_agent_name"),
  direct_reports_included:   jsonb("direct_reports_included").$type<number[]>().default([]),
  top_insights:              jsonb("top_insights").$type<any[]>().default([]),
  top_ideas:                 jsonb("top_ideas").$type<any[]>().default([]),
  top_actions:               jsonb("top_actions").$type<any[]>().default([]),
  top_cowork_requests:       jsonb("top_cowork_requests").$type<any[]>().default([]),
  conflicts:                 jsonb("conflicts").$type<any[]>().default([]),
  boss_summary:              text("boss_summary"),
  created_at:                text("created_at").notNull(),
});
export type BossConsolidation = typeof bossConsolidations.$inferSelect;

export const ceoBriefs = pgTable("ceo_briefs", {
  id:                  serial("id").primaryKey(),
  cycle_id:            integer("cycle_id").notNull(),
  ...
  top_cowork_requests: jsonb("top_cowork_requests").$type<any[]>().default([]),
  conflicts:           jsonb("conflicts").$type<any[]>().default([]),
  decisions_required:  jsonb("decisions_required").$type<any[]>().default([]),
  autonomous_actions:  jsonb("autonomous_actions").$type<any[]>().default([]),
  coo_proposals:       jsonb("coo_proposals").$type<any[]>().default([]),
  cowork_prompt:       text("cowork_prompt"),
  created_at:          text("created_at").notNull(),
});
export type CeoBrief = typeof ceoBriefs.$inferSelect;

// parsed_status: not_parsed | parsed | failed
export const coworkOutputs = pgTable("cowork_outputs", {
  id:              serial("id").primaryKey(),
  cycle_id:        integer("cycle_id").notNull(),
  raw_output_text: text("raw_output_text").notNull(),
  pasted_by:       text("pasted_by").notNull().default("President"),
  pasted_at:       text("pasted_at").notNull(),
  parsed_status:   text("parsed_status").notNull().default("not_parsed"),
  created_at:      text("created_at").notNull(),
});
export type CoworkOutput = typeof coworkOutputs.$inferSelect;

// status: received | assigned | processed
export const coworkLetters = pgTable("cowork_letters", {
  id:                        serial("id").primaryKey(),
  cycle_id:                  integer("cycle_id").notNull(),
  agent_id:                  integer("agent_id"),
  agent_name:                text("agent_name"),
  raw_letter_text:           text("raw_letter_text").notNull(),
  extracted_findings:        jsonb("extracted_findings").$type<string[]>().default([]),
  extracted_recommendations: jsonb("extracted_recommendations").$type<string[]>().default([]),
...
// These are standalone daily briefs generated by the ceoBriefRunner service,
// with full structured decision cards that Livio can approve/reject/modify.
export const ceoBriefRuns = pgTable("ceo_brief_runs", {
  id:                  uuid("id").primaryKey().defaultRandom(),
  generatedAt:         timestamp("generated_at", { withTimezone: true }).defaultNow().notNull(),
  generatedBy:         text("generated_by").notNull(),              // 'manual' | 'scheduled'
  rawBriefMarkdown:    text("raw_brief_markdown"),
  claudeResponseRaw:   text("claude_response_raw"),
  model:               text("model"),
...
export type CeoBriefRun = typeof ceoBriefRuns.$inferSelect;

export const ceoBriefRunDecisions = pgTable("ceo_brief_run_decisions", {
  id:              uuid("id").primaryKey().defaultRandom(),
  briefId:         uuid("brief_id").notNull().references(() => ceoBriefRuns.id),
  decisionId:      text("decision_id").notNull(),
  type:            text("type").notNull(),               // 'idea' | 'action' | 'conflict' | 'proposal'
  agent:           text("agent").notNull(),
  title:           text("title").notNull(),
  description:     text("description").notNull(),
  okrLink:         text("okr_link"),
  deadline:        text("deadline"),
  approvalLevel:   text("approval_level").notNull(),     // 'autonomous' | 'boss' | 'ceo' | 'livio'
  impact:          integer("impact"),
  effort:          integer("effort"),
  risk:            integer("risk"),
  status:          text("status").notNull().default("pending"), // 'pending' | 'approved' | 'rejected' | 'modified' | 'postponed'
  statusNote:      text("status_note"),
  modifiedText:    text("modified_text"),
  postponeUntil:   text("postpone_until"),
